...
public class DiscountService {
    ...
    public BigDecimal calculateDiscount(BigDecimal amount, String userType, boolean isGrocery, LocalDateTime customerSince) {
        ...
        try {
			...
			BigDecimal extraDiscount = amount.divideToIntegralValue(BigDecimal.valueOf(100)).multiply(BigDecimal.valueOf(5));
			discount = discount.add(extraDiscount);
			logger.info("CalculateDiscount Method: userType: {} has got discount of {} on amount: {}", userType, discount, amount);
	        
		} catch (Exception ex) {
            logger.error("Exception while calculating discount: {}", ex.getMessage());
            throw new CustomException("Discount calculation failed. Reason-" + ex.getMessage());
        }
